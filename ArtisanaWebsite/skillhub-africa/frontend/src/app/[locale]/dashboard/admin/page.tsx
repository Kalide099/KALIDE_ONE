...
export default function SupremeAdminDashboard() {
  ...
  if (isLoading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center font-black uppercase tracking-widest text-red-500 animate-pulse">{t.Admin?.initializing || 'Initializing Supreme Control Node...'}</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 flex overflow-hidden">
      ...
      <aside className="bg-white shadow-sm border border-gray-100 w-64 h-screen border-r border-red-500/20 shadow-[30px_0_30px_rgba(220,38,38,0.02)] flex flex-col z-50 shrink-0">
        <div className="h-24 p-6 flex items-center space-x-4 border-b border-gray-200 shrink-0">
          <div className="w-12 h-12 rounded-xl bg-red-600 flex items-center justify-center font-black text-gray-900 shadow-[0_0_15px_rgba(220,38,38,0.5)]">
            K
          </div>
            ...
            <button 
              ...
              className={`w-full text-left px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-between ${
                activeTab === item 
                  ...
                  : 'text-gray-500 border border-transparent hover:bg-gray-100 hover:text-red-400'
              }`}
            >
              <span>{item}</span>
        ...
        <div className="p-6 border-t border-gray-200 shrink-0">
          <button 
            ...
            className="w-full px-4 py-3 border border-red-500/30 text-red-400 hover:bg-red-500 hover:text-gray-900 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
          >
      ...
      <div className="flex-1 h-screen overflow-y-auto relative scrollbar-hide">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {[
            ...
            <div key={idx} className="bg-white shadow-sm border border-gray-100 p-8 rounded-[2rem] border-red-500/10 relative overflow-hidden group">
        ...
        {/* Tab Content */}
        {activeTab === 'Users' && (
          <div className="bg-white shadow-sm border border-gray-100 rounded-[3rem] p-8 md:p-12 border-red-500/20 shadow-[0_0_30px_rgba(220,38,38,0.05)] animate-in fade-in zoom-in duration-500">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody>
                  {users.map(user => (
                    <tr key={user.id} className="border-b border-gray-200 hover:bg-red-500/5 transition-colors">
                      <td className="py-6 text-sm font-bold text-gray-500">#{user.id}</td>
                      ...
                      <td className="py-6 flex justify-end space-x-2">
                         <button 
                          ...
                          className="px-3 py-2 border border-blue-500/30 text-blue-400 hover:bg-blue-500 hover:text-gray-900 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                        >
                          {t.Admin?.actions?.dossier}
                        </button>
                        <button 
                          onClick={() => handleToggleAccess(user.id)}
                          className="px-3 py-2 border border-slate-500/30 text-gray-500 hover:bg-slate-500 hover:text-gray-900 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                        >
                          {user.is_active ? t.Admin?.actions?.suspend : t.Admin?.actions?.unsuspend}
                        </button>
                        <button 
                          onClick={() => handleWarn(user.id, user.name)}
                          className="px-3 py-2 border border-orange-500/30 text-orange-400 hover:bg-orange-500 hover:text-gray-900 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                        >
                          {t.Admin?.actions?.warn}
                        </button>
                        <button 
                          onClick={() => handleDelete(user.id)}
                          className="px-3 py-2 bg-red-600/20 text-red-500 hover:bg-red-600 hover:text-gray-900 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all shadow-lg"
                        >
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {projects.map(project => (
                    <tr key={project.id} className="border-b border-gray-200 hover:bg-gray-100 transition-colors">
                      <td className="py-6 text-sm font-bold text-gray-500">#{project.id}</td>
        ...
        {!['Users', 'Projects', 'Services', 'Supply'].includes(activeTab) && (
          <div className="bg-white shadow-sm border border-gray-100 rounded-[3rem] p-8 md:p-12 border-red-500/20 shadow-[0_0_30px_rgba(220,38,38,0.05)] flex flex-col items-center justify-center min-h-[400px] animate-in fade-in zoom-in duration-500">
            ...
            <p className="text-sm font-bold text-gray-500 uppercase tracking-widest text-center max-w-lg leading-relaxed">
              {t.Admin?.offlineDesc}
            </p>
          </div>
        )}
        ...
        {selectedDossier && (
          <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-6 backdrop-blur-md">
            <div className="bg-white shadow-sm border border-gray-100 w-full max-w-2xl rounded-[3rem] p-8 border-red-500/30 relative">
              <button 
                onClick={() => setSelectedDossier(null)}
                className="absolute top-8 right-8 text-gray-500 hover:text-gray-900 font-black text-xl"
              >
                X
              </button>
              ...
              <div className="grid grid-cols-2 gap-6 mb-8 text-sm">
                <div className="bg-black/40 p-4 rounded-xl border border-gray-200">
              ...
              <div className="border-t border-gray-200 pt-8">
                ...
                <div className="flex space-x-4">
                  <button onClick={() => handleUpgrade(selectedDossier.id, 'free')} className="px-4 py-2 border border-slate-500/30 text-gray-500 hover:bg-slate-500 hover:text-gray-900 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all">
                    {t.Admin?.dossier?.grantFree}
                  </button>
                  <button onClick={() => handleUpgrade(selectedDossier.id, 'pro')} className="px-4 py-2 border border-blue-500/30 text-blue-400 hover:bg-blue-500 hover:text-gray-900 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all">
                    {t.Admin?.dossier?.grantPro}
                  </button>
                  <button onClick={() => handleUpgrade(selectedDossier.id, 'elite')} className="px-4 py-2 border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500 hover:text-gray-900 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all">
