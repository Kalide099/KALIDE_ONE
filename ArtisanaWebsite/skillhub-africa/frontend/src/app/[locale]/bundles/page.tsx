...
export default function Bundles() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 pt-32 pb-24 px-6 relative overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="mb-20">
          <div className="inline-block px-4 py-1.5 bg-white shadow-sm border border-gray-100 rounded-full text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-6 animate-pulse">
          ...
          <p className="text-gray-500 font-medium max-w-2xl text-lg uppercase tracking-widest leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {bundles.map((bundle) => (
            <div key={bundle.id} className="group bg-white shadow-sm border border-gray-100 p-10 rounded-[3rem] border-gray-200 hover:border-primary/40 transition-all duration-500 flex flex-col h-full hover:-translate-y-2">
              <div className="flex justify-between items-start mb-8">
                <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center text-4xl group-hover:scale-110 transition-transform">
                  {bundle.icon}
                </div>
              ...
              <div className="mb-8">
                <h3 className="text-3xl font-black uppercase tracking-tight mb-2 text-gray-900 italic">{bundle.title}</h3>
                <p className="text-primary text-[10px] font-black uppercase tracking-widest mb-4">{bundle.subtitle}</p>
                <p className="text-gray-500 text-sm font-medium leading-relaxed flex-1">{bundle.desc}</p>
              </div>

              <div className="mt-auto pt-8 border-t border-gray-200">
                <button 
                  onClick={handleInitiate}
                  className="w-full py-4 bg-white text-black hover:bg-primary hover:text-gray-900 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-xl hover:shadow-primary/30"
                >
